...
import numpy as np
import cv2
from d435_helpers_without_pyrealsense import *
...
def exposure_argument_is_valid(value):
    ...
    if is_string:
        ...
        int_value = value
    if exposure_range[0] <= int_value and int_value <= exposure_range[1]:
        return True
    return False
...
def start_d435(exposure): 
    ...
    for info in camera_info:
        if 'D435' in info['name']:
            d435_info = info
    if d435_info is None:
        print('D435 camera not found')
        print('Exiting')
        exit()
    else:
        print('D435 found:')
    ...
    config = rs.config()
    config.enable_device(d435_info['serial_number'])
